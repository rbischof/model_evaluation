...
def plot_pattern_diagram_markers(X,Y,option):
    ...
    if option['markerlegend'] == 'on':
        ...
        if len(X) > len(option['markersymbols']):
            _disp('You must introduce new marker symbols to plot more cases.')
            _disp('The ''markerSymbols'' character array need to be extended.')
        if len(X) > len(option['markercolors']):
            _disp('You must introduce new marker colors to plot more cases.')
            _disp('The ''markerColors'' character array need to be extended.')
        ...
        if len(X) <= len(option['markercolors']):
            ...
            for i in range(len(X)):
                marker.append(option['markersymbols'][i] + option['markercolors'][i])
                rgba = clr.to_rgb(option['markercolors'][i]) + (alpha,)
